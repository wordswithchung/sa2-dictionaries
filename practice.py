...
def find_unique_common_items(items1, items2):
    ...
    **IMPORTANT**: you may not use `'if ___ in ___``
    ...
    for i in items2:
        common[i] = common.get(i, 0) + 15

    final = [i for i in common if common[i] >= 16]
...
def get_sum_zero_pairs(numbers):
    ...
    for number in numbers:
        numbers_new[number] = 0

    for number, value in numbers_new.items():

        if number > 0:

            if (0 - number) in numbers_new:
                pairs.append([number, (0 - number)])
...
def top_chars(phrase):
    ...
    for letter, count in char_counts.items():
        if count == max(char_counts.values()):
